...
export async function getUserCandidate(candidateIdList: string[]) {
    type userCandidate = Pick<User, "id_number" | 'firstname' | 'lastname' | 'year_level' | 'section' | 'course'> & Pick<Candidate, 'candidate_id' | 'election_id' | 'position' | 'enabled' | 'alias' | 'party'>;
        ...
        ORDER BY u.lastname;
        `
        const userCandidateResult = await selectQuery<userCandidate>(pool, sqlSelectUserCandidateQuery, [candidateIdList]);
